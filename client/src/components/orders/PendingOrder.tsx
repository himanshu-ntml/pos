...
export function PendingOrder({ order }: PendingOrderProps) {
  ...
  const tableNumber = venueTables?.find((table) => table.id === order.tableId)?.number;
      ...
      <CardHeader className="pb-5">
        <CardTitle className="flex justify-between">
          ...
          <span className="flex gap-2">
            ...
            {order.isPaid && (
              <Badge variant="secondary" className="bg-green-300/20 border-green-500">
                Paid
              </Badge>
            )}
          </span>
        ...
        <div className="flex items-center gap-4  justify-between">
          <div className="grid gap-1 text-sm grid-flow-col">
            {tableNumber ? (
              ...
              <p key="table not selected" className="font-semibold  text-sm flex">
                <HandPlatter className="h-6 w-6 mr-2" />
                Take Away
              </p>
            )}
            {!!order?.specialRequest && (
              <p key="special request table" className="text-sm text-gray-500 dark:text-gray-400">
